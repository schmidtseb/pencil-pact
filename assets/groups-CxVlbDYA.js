import{d as e}from"./index-ysYX02KG.js";async function t(t){let{data:n,error:r}=await e.from(`group_members`).select(`group_id, role`).eq(`user_id`,t);if(r)throw r;if(!n||n.length===0)return[];let i=n.map(e=>e.group_id),{data:a,error:o}=await e.from(`groups`).select(`
      *,
      group_members (
        id,
        user_id,
